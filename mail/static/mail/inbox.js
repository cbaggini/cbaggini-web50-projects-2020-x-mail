...
function load_mailbox(mailbox) {
  ...
  fetch(`/emails/${mailbox}`)
    ...
    .then(result => {
      ...
      result.forEach(el => {
        if (el.read) {
          var i = `<div id="${el.id}" class="bg-secondary text-white border border-dark my-3 p-3 rounded-lg d-flex flex-row justify-content-between"> <div> <strong>${el.sender}</strong>   ${el.subject}</div> <div> ${el.timestamp} </div></div>`
          emails += i;
        } else {
          var i = `<div id="${el.id}" class="border border-dark my-3 p-3 rounded-lg d-flex flex-row justify-content-between"> <div> <strong>${el.sender}</strong>   ${el.subject}</div> <div> ${el.timestamp} </div></div>`
          emails += i;
        }
      });
